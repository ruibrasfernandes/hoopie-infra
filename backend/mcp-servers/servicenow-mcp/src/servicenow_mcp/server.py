...
class ServiceNowMCP:
    ...
    def _determine_enabled_tools(self):
        ...
        logger.info(f"=== TOOL PACKAGE DETERMINATION ===")
        logger.info(f"MCP_TOOL_PACKAGE environment variable: '{requested_package}'")
        logger.info(f"Available packages: {list(self.package_definitions.keys())}")
        ...
        logger.info(f"Enabled tool names count: {len(self.enabled_tool_names)}")
        if self.enabled_tool_names:
            logger.info(f"First 10 enabled tools: {self.enabled_tool_names[:10]}")

        logger.info(
            f"Loading package '{self.current_package_name}' with {len(self.enabled_tool_names)} tools."
        )
        logger.info(f"=== END TOOL PACKAGE DETERMINATION ===")

    async def _list_tools_impl(self) -> List[types.Tool]:
        """Implementation for the list_tools MCP endpoint."""
        logger.info("Starting _list_tools_impl...")
    ...
    async def _call_tool_impl(self, name: str, arguments: dict) -> list[types.TextContent]:
        ...
        try:
            # Clean arguments - remove None and empty string values so defaults can be applied
            cleaned_arguments = {k: v for k, v in arguments.items() if v is not None and v != ""}
            params = params_model(**cleaned_arguments)
            logger.info(f"Parsed arguments for tool '{name}': {params}")
